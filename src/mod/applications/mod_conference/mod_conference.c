...
SWITCH_MODULE_DEFINITION (mod_conference, mod_conference_load,
			  mod_conference_shutdown, NULL);
...
static conference_relationship_t *member_get_relationship (conference_member_t
							   * member,
							   conference_member_t
							   * other_member);
static conference_member_t *conference_member_get (conference_obj_t *
						   conference, uint32_t id);
static conference_relationship_t *member_add_relationship (conference_member_t
							   * member,
							   uint32_t id);
static switch_status_t member_del_relationship (conference_member_t * member,
						uint32_t id);
static switch_status_t conference_add_member (conference_obj_t * conference,
					      conference_member_t * member);
static switch_status_t conference_del_member (conference_obj_t * conference,
					      conference_member_t * member);
static void *SWITCH_THREAD_FUNC conference_thread_run (switch_thread_t *
						       thread, void *obj);
static void *SWITCH_THREAD_FUNC conference_video_thread_run (switch_thread_t *
							     thread,
							     void *obj);
static void conference_loop_output (conference_member_t * member);
static uint32_t conference_stop_file (conference_obj_t * conference,
				      file_stop_t stop);
static switch_status_t conference_play_file (conference_obj_t * conference,
					     char *file, uint32_t leadin,
					     switch_channel_t * channel,
					     uint8_t async);
static switch_status_t conference_say (conference_obj_t * conference,
				       const char *text, uint32_t leadin);
static void conference_list (conference_obj_t * conference,
			     switch_stream_handle_t * stream, char *delim);
...
static void *SWITCH_THREAD_FUNC conference_loop_input (switch_thread_t *
						       thread, void *obj);
static switch_status_t conference_local_play_file (conference_obj_t *
						   conference,
						   switch_core_session_t *
						   session, char *path,
						   uint32_t leadin, void *buf,
						   uint32_t buflen);
static switch_status_t conference_member_play_file (conference_member_t *
						    member, char *file,
						    uint32_t leadin);
static switch_status_t conference_member_say (conference_member_t * member,
					      char *text, uint32_t leadin);
static uint32_t conference_member_stop_file (conference_member_t * member,
					     file_stop_t stop);
static conference_obj_t *conference_new (char *name, conf_xml_cfg_t cfg,
					 switch_memory_pool_t * pool);
static switch_status_t chat_send (char *proto, char *from, char *to,
				  char *subject, char *body, char *hint);
static void launch_conference_record_thread (conference_obj_t * conference,
					     char *path);

typedef switch_status_t (*conf_api_args_cmd_t) (conference_obj_t *,
						switch_stream_handle_t *, int,
						char **);
typedef switch_status_t (*conf_api_member_cmd_t) (conference_member_t *,
						  switch_stream_handle_t *,
						  void *);
typedef switch_status_t (*conf_api_text_cmd_t) (conference_obj_t *,
						switch_stream_handle_t *,
						const char *);
...
static switch_status_t conf_api_sub_undeaf (conference_member_t * member,
					    switch_stream_handle_t * stream,
					    void *data);
static switch_status_t conference_add_event_data (conference_obj_t *
						  conference,
						  switch_event_t * event);
static switch_status_t conference_add_event_member_data (conference_member_t *
							 member,
							 switch_event_t *
							 event);

static switch_status_t
conference_add_event_data (conference_obj_t * conference,
			   switch_event_t * event)
{
  ...
  switch_event_add_header (event, SWITCH_STACK_BOTTOM, "Conference-Size",
			   "%u", conference->count);
  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
...
static switch_status_t
conference_add_event_member_data (conference_member_t * member,
				  switch_event_t * event)
{
  switch_status_t status = SWITCH_STATUS_SUCCESS;
  ...
  if (member->session)
    {
      switch_channel_t *channel =
	switch_core_session_get_channel (member->session);
  ...
  switch_event_add_header (event, SWITCH_STACK_BOTTOM, "Member-ID", "%u",
			   member->id);
...
static conference_relationship_t *
member_get_relationship (conference_member_t * member,
...
conference_member_get (conference_obj_t * conference, uint32_t id)
{
...
static switch_status_t
conference_record_stop (conference_obj_t * conference, char *path)
{
  conference_member_t *member = NULL;
  int count = 0;
  ...
  for (member = conference->members; member; member = member->next)
    {
      if (switch_test_flag (member, MFLAG_NOCHANNEL)
	  && (!path || !strcmp (path, member->rec_path)))
	{
...
static conference_relationship_t *
member_add_relationship (conference_member_t * member, uint32_t id)
{
  conference_relationship_t *rel = NULL;

  if (member == NULL || id == 0
      || !(rel = switch_core_alloc (member->pool, sizeof (*rel))))
    return NULL;
...
static switch_status_t
member_del_relationship (conference_member_t * member, uint32_t id)
{
...
static switch_status_t
conference_add_member (conference_obj_t * conference,
		       conference_member_t * member)
{
  switch_status_t status = SWITCH_STATUS_FALSE;
  switch_event_t *event;
  ...
  if (!switch_test_flag (member, MFLAG_NOCHANNEL))
    {
      conference->count++;
      if (switch_event_create (&event, SWITCH_EVENT_PRESENCE_IN) ==
	  SWITCH_STATUS_SUCCESS)
	{
	  ...
	  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "login",
					  conference->name);
	  switch_event_add_header (event, SWITCH_STACK_BOTTOM, "from",
				   "%s@%s", conference->name,
				   conference->domain);
	  switch_event_add_header (event, SWITCH_STACK_BOTTOM, "status",
				   "Active (%d caller%s)", conference->count,
				   conference->count == 1 ? "" : "s");
	  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					  "event_type", "presence");
	  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					  "alt_event_type", "dialog");
	  switch_event_add_header (event, SWITCH_STACK_BOTTOM, "event_count",
				   "%d", EC++);
	  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					  "unique-id", conference->name);
	  switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
      ...
      if (conference->count > 1)
	{
	  ...
	  if (conference->enter_sound)
	    {
	      conference_play_file (conference, conference->enter_sound,
				    CONF_DEFAULT_LEADIN,
				    switch_core_session_get_channel (member->
								     session),
				    1);
	    }
	}
      ...
      call_list =
	(call_list_t *) switch_channel_get_private (channel,
						    "_conference_autocall_list_");

      if (call_list)
	{
	  char saymsg[1024];
	  switch_snprintf (saymsg, sizeof (saymsg), "Auto Calling %d parties",
			   call_list->itteration);
	  ...
	  if (switch_strlen_zero (conference->special_announce))
	    {
	      ...
	      if (conference->count >= conference->announce_count
		  && conference->announce_count > 1)
		{
		  switch_snprintf (msg, sizeof (msg), "There are %d callers",
				   conference->count);
	      ...
	      else if (conference->count == 1 && !conference->perpetual_sound)
		{
		  if (conference->alone_sound)
		    {
		      conference_stop_file (conference, FILE_STOP_ASYNC);
		      conference_play_file (conference,
					    conference->alone_sound,
					    CONF_DEFAULT_LEADIN,
		    ...
		    {
		      switch_snprintf (msg, sizeof (msg),
				       "You are currently the only person in this conference.");
		      conference_member_say (member, msg,
					     CONF_DEFAULT_LEADIN);
		    }
		}
	    }
      ...
      if (conference->min && conference->count >= conference->min)
	{
	  ...
	  switch_event_create_subclass (&event, SWITCH_EVENT_CUSTOM,
					CONF_EVENT_MAINT) ==
	  SWITCH_STATUS_SUCCESS)
	{
...
static switch_status_t
conference_del_member (conference_obj_t * conference,
		       conference_member_t * member)
{
  switch_status_t status = SWITCH_STATUS_FALSE;
  ...
  if (!switch_test_flag (member, MFLAG_NOCHANNEL))
	  ...
	  SWITCH_STATUS_SUCCESS)
	{
      ...
      if ((conference->min && switch_test_flag (conference, CFLAG_ENFORCE_MIN)
	   && conference->count < conference->min)
	  || (switch_test_flag (conference, CFLAG_DYNAMIC)
	      && conference->count == 0))
	{
...
conference_video_thread_run (switch_thread_t * thread, void *obj)
{
  ...
  while (has_vid && conference->video_running == 1 && globals.running
      ...
      if (!conference->floor_holder)
	{
      ...
      if (switch_channel_test_flag
	  (switch_core_session_get_channel
	   (conference->floor_holder->session), CF_VIDEO))
	{
	  status =
	    switch_core_session_read_video_frame (conference->floor_holder->
						  session, &vid_frame,
	  ...
	  if (conference->floor_holder != last_member)
	      ...
	      if (!req_iframe)
		{
		  ...
		  switch_core_session_receive_message (conference->
						       floor_holder->session,
						       &msg);
		  req_iframe = 1;
		}
#endif

	      if (vid_frame->codec->implementation->ianacode == 34)
		{		/* h.263 */
		  iframe = (*((int16_t *) vid_frame->data) >> 12 == 6);
		}
	      else if (vid_frame->codec->implementation->ianacode == 115)
		{		/* h.263-1998 */
		  int y = *((int8_t *) vid_frame->data + 2) & 0xfe;
		  iframe = (y == 0x80 || y == 0x82);
		}
	      else if (vid_frame->codec->implementation->ianacode == 99)
		{		/* h.264 */
	  ...
	  switch_mutex_lock (conference->member_mutex);
	  has_vid = 0;
	  for (imember = conference->members; imember;
		...
		{
		  has_vid++;
		  switch_core_session_write_video_frame (imember->session,
							 vid_frame,
							 SWITCH_IO_FLAG_NONE,
							 0);
		}
  ...
  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
		     "Video thread ending for conference %s\n",
		     conference->name);
...
static void
launch_conference_video_thread (conference_obj_t * conference)
{
  ...
  switch_thread_create (&thread, thd_attr, conference_video_thread_run,
			conference, conference->pool);
}

/* Main monitor thread (1 per distinct conference room) */
static void *SWITCH_THREAD_FUNC
conference_thread_run (switch_thread_t * thread, void *obj)
{
  conference_obj_t *conference = (conference_obj_t *) obj;
  conference_member_t *imember, *omember;
  uint32_t samples =
    switch_samples_per_packet (conference->rate, conference->interval);
  ...
  file_frame =
    switch_core_alloc (conference->pool, SWITCH_RECOMMENDED_BUFFER_SIZE);
  async_file_frame =
    switch_core_alloc (conference->pool, SWITCH_RECOMMENDED_BUFFER_SIZE);

  if (switch_core_timer_init
      (&timer, conference->timer_name, conference->interval, samples,
  ...
  while (globals.running && !switch_test_flag (conference, CFLAG_DESTRUCT))
	...
	{
	  conference_play_file (conference, conference->perpetual_sound,
				CONF_DEFAULT_LEADIN, NULL, 1);
	}
      else if (conference->moh_sound && conference->count == 1
	       && !conference->async_fnode)
	{
	  conference_play_file (conference, conference->moh_sound,
				CONF_DEFAULT_LEADIN, NULL, 1);
	}
      ...
      for (imember = conference->members; imember; imember = imember->next)
	{
	  ...
	  if (imember->session
	      &&
	      switch_channel_test_flag (switch_core_session_get_channel
					(imember->session), CF_VIDEO))
	    {
	      members_with_video++;
	    }
	  ...
	  if (switch_buffer_inuse (imember->audio_buffer) >= bytes
	      && (buf_read =
		  (uint32_t) switch_buffer_read (imember->audio_buffer,
						 imember->frame, bytes)))
	    {
      ...
      if (conference->fnode)
	{
	  ...
	  if (conference->fnode->leadin)
	      ...
	      if (conference->fnode->type == NODE_TYPE_SPEECH)
		{
		  ...
		  if (switch_core_speech_read_tts
		      (conference->fnode->sh, file_frame, &file_data_len,
		       &rate, &flags) == SWITCH_STATUS_SUCCESS)
		    {
      ...
      if (conference->async_fnode)
	{
	  ...
	  if (conference->async_fnode->leadin)
	    ...
	    {
	      file_sample_len = samples;
	      switch_core_file_read (&conference->async_fnode->fh,
				     async_file_frame, &file_sample_len);
	      ...
	      else
		{
		  if (has_file_data)
		    ...
		    {
		      memcpy (file_frame, async_file_frame,
			      file_sample_len * 2);
		      has_file_data = 1;
		    }
		}
      ...
      if (ready || has_file_data)
	{
	  ...
	  for (omember = conference->members; omember;
	       omember = omember->next)
	    {
	      if (has_file_data && file_sample_len)
		{
		  ...
		  if (sample_bytes < bytes)
			...
			{
			  switch_generate_sln_silence ((int16_t *) omember->
						       mux_frame +
						       sample_bytes,
						       (bytes -
							sample_bytes) / 2,
			...
			{
			  memset (omember->mux_frame + sample_bytes, 255,
				  bytes - sample_bytes);
			}
		    }
		}
		   ...
		   imember = imember->next)
		{
...
#if 0
		  if (nt && conference->not_talking_buf_len
		      && !switch_test_flag (omember, MFLAG_HAS_AUDIO))
		    {
		      memcpy (omember->mux_frame, conference->not_talking_buf,
			      conference->not_talking_buf_len);
		      continue;
		    }
#endif
		  ...
		  if (total - ready > 1)
			...
			{
			  conference->not_talking_buf =
			    switch_core_alloc (conference->pool,
					       imember->read + 128);
			}
		      memcpy (conference->not_talking_buf, muxed,
			      conference->not_talking_buf_len);
		      nt++;
		    }
#endif
		  ...
		  if (switch_test_flag (imember, MFLAG_RUNNING))
		    {
		      ...
		      ok =
			switch_buffer_write (imember->mux_buffer,
					     imember->mux_frame, bytes);
  ...
  if (switch_event_create (&event, SWITCH_EVENT_PRESENCE_IN) ==
      SWITCH_STATUS_SUCCESS)
    {
      ...
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "login",
				      conference->name);
      switch_event_add_header (event, SWITCH_STACK_BOTTOM, "from", "%s@%s",
			       conference->name, conference->domain);
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "status",
				      "Inactive");
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "rpid",
				      "idle");
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
				      "event_type", "presence");
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
	  ...
	  if (switch_test_flag (conference, CFLAG_ANSWERED))
	  ...
	  else
	    {
	      ...
	      switch_channel_hangup (channel,
				     conference->bridge_hangup_cause);
	    }
	}
...
static void
conference_loop_fn_mute_toggle (conference_member_t * member,
...
static void
conference_loop_fn_deafmute_toggle (conference_member_t * member,
				    caller_control_action_t * action)
{
  if (member == NULL)
    return;
...
static void
conference_loop_fn_energy_up (conference_member_t * member,
			      caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
  ...
  if (test_eflag (member->conference, EFLAG_ENERGY_LEVEL) &&
    ...
    {
      conference_add_event_member_data (member, event);
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "energy-level");
      switch_event_add_header (event, SWITCH_STACK_BOTTOM, "New-Level", "%d",
			       member->energy_level);
      switch_event_fire (&event);
    }
  switch_mutex_unlock (member->flag_mutex);

  switch_snprintf (msg, sizeof (msg), "Energy level %d",
		   member->energy_level);
  conference_member_say (member, msg, 0);
}

static void
conference_loop_fn_energy_equ_conf (conference_member_t * member,
				    caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_energy_dn (conference_member_t * member,
			      caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_volume_talk_up (conference_member_t * member,
				   caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
  ...
  switch_snprintf (msg, sizeof (msg), "Volume level %d",
		   member->volume_out_level);
  conference_member_say (member, msg, 0);
}

static void
conference_loop_fn_volume_talk_zero (conference_member_t * member,
				     caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_volume_talk_dn (conference_member_t * member,
				   caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_volume_listen_up (conference_member_t * member,
				     caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
  ...
  switch_snprintf (msg, sizeof (msg), "Gain level %d",
		   member->volume_in_level);
  conference_member_say (member, msg, 0);
}

static void
conference_loop_fn_volume_listen_zero (conference_member_t * member,
				       caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_volume_listen_dn (conference_member_t * member,
				     caller_control_action_t * action)
{
  char msg[512];
  switch_event_t *event;
...
static void
conference_loop_fn_event (conference_member_t * member,
			  caller_control_action_t * action)
{
  switch_event_t *event;
  if (test_eflag (member->conference, EFLAG_DTMF)
      ...
      SWITCH_STATUS_SUCCESS)
    {
      ...
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "dtmf");
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "DTMF-Key",
				      action->binded_dtmf);
      switch_event_fire (&event);
    }
}
...
static void
conference_loop_fn_hangup (conference_member_t * member,
			   caller_control_action_t * action)
{
  switch_clear_flag_locked (member, MFLAG_RUNNING);
}
...
static void *SWITCH_THREAD_FUNC
conference_loop_input (switch_thread_t * thread, void *obj)
{
  ...
  switch_codec_t *read_codec;
  uint32_t hangover = 40, hangunder = 15, hangover_hits = 0, hangunder_hits =
    0, energy_level = 0, diff_level = 400;
  ...
  while (switch_test_flag (member, MFLAG_RUNNING)
	 && switch_channel_ready (channel))
    {
      /* Read a frame. */
      status =
	switch_core_session_read_frame (member->session, &read_frame,
					SWITCH_IO_FLAG_NONE, 0);
	  ...
	  if (switch_test_flag (member, MFLAG_TALKING))
	      ...
	      if (++hangover_hits >= hangover)
		{
		  ...
		  if (test_eflag (member->conference, EFLAG_STOP_TALKING) &&
		      switch_event_create_subclass (&event,
						    SWITCH_EVENT_CUSTOM,
						    CONF_EVENT_MAINT) ==
		      SWITCH_STATUS_SUCCESS)
		    {
      ...
      if (switch_test_flag (member, MFLAG_CAN_SPEAK) && energy_level)
	{
	  ...
	  if (!
	      (divisor =
	       read_codec->implementation->actual_samples_per_second / 8000))
	    {
	      divisor = 1;
	    }
	  ...
	  if (member->score > energy_level)
	      ...
	      if (diff >= diff_level || ++hangunder_hits >= hangunder)
		{
		  ...
		  if (!switch_test_flag (member, MFLAG_TALKING))
			...
			{
			  if (test_eflag
			      (member->conference, EFLAG_FLOOR_CHANGE)
			      && switch_event_create_subclass (&event,
							       SWITCH_EVENT_CUSTOM,
							       CONF_EVENT_MAINT)
			      == SWITCH_STATUS_SUCCESS)
			    {
			      conference_add_event_member_data (member,
								event);
			      switch_event_add_header_string (event,
							      SWITCH_STACK_BOTTOM,
							      "Action",
							      "floor-change");
			      switch_event_add_header (event,
						       SWITCH_STACK_BOTTOM,
						       "Old-ID", "%d",
						       member->conference->
						       floor_holder ? member->
      ...
      if ((switch_test_flag (member, MFLAG_TALKING) || energy_level == 0)
	  && switch_test_flag (member, MFLAG_CAN_SPEAK))
	{
	  ...
	  if (read_resampler)
		...
		switch_short_to_float (bptr, read_resampler->from,
				       (int) len / 2);
	      read_resampler->to_len =
		switch_resample_process (read_resampler, read_resampler->from,
					 read_resampler->from_len,
					 read_resampler->to,
					 read_resampler->to_size, 0);

	      switch_float_to_short (read_resampler->to, member->resample_out,
				     read_resampler->to_len);
	  ...
	  if (member->volume_in_level)
	    {
	      switch_change_sln_volume (data, datalen / 2,
					member->volume_in_level);
	    }
...
static void
launch_conference_loop_input (conference_member_t * member,
			      switch_memory_pool_t * pool)
{
  switch_thread_t *thread;
  switch_threadattr_t *thd_attr = NULL;
  ...
  switch_thread_create (&thread, thd_attr, conference_loop_input, member,
			pool);
}
...
static caller_control_fn_table_t ccfntbl[] = {
  ...
  {"vol listen zero", "5", CALLER_CONTROL_VOL_LISTEN_ZERO,
   conference_loop_fn_volume_listen_zero},
...
static void
conference_loop_output (conference_member_t * member)
{
  switch_channel_t *channel =
    switch_core_session_get_channel (member->session);
  ...
  switch_timer_t timer = { 0 };
  switch_codec_t *read_codec =
    switch_core_session_get_read_codec (member->session);
  uint32_t interval =
    read_codec->implementation->microseconds_per_packet / 1000;
  uint32_t samples =
    switch_samples_per_packet (member->conference->rate, interval);
  uint32_t csamples = samples;
  uint32_t tsamples =
    member->orig_read_codec->implementation->samples_per_packet;
  ...
  flush_len =
    switch_samples_per_packet (member->conference->rate,
			       member->conference->interval) * 10;

  if (switch_core_timer_init
      (&timer, member->conference->timer_name, interval, tsamples,
  ...
  write_frame.data = data =
    switch_core_session_alloc (member->session,
  ...
  launch_conference_loop_input (member,
				switch_core_session_get_pool (member->
							      session));

  /* build a digit stream object */
  if (member->conference->dtmf_parser != NULL
      && switch_ivr_digit_stream_new (member->conference->dtmf_parser,
				      &member->digit_stream) !=
      SWITCH_STATUS_SUCCESS)
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
  ...
  if ((call_list =
	...
	switch_channel_get_variable (channel,
      ...
      if (ann)
	{
	  member->conference->special_announce =
	    switch_core_strdup (member->conference->pool, ann);
	}
	...
	{
	  conference_outcall_bg (member->conference, NULL, NULL, cp->string,
				 to, switch_str_nil (flags), cid_name,
				 cid_num);
	}
    }
  ...
  while (switch_test_flag (member, MFLAG_RUNNING)
	 ...
	 && switch_channel_ready (channel))
    {
      ...
      if (switch_core_session_dequeue_event (member->session, &event) ==
	  SWITCH_STATUS_SUCCESS)
	{
	  if (event->event_id == SWITCH_EVENT_MESSAGE)
		...
		{
		  if ((p = strchr (to, '+'))
		      && strncmp (to, CONF_CHAT_PROTO,
				  strlen (CONF_CHAT_PROTO)))
		    {
		      freeme =
			switch_mprintf ("%s+%s@%s", CONF_CHAT_PROTO,
      ...
      if (switch_channel_test_flag (channel, CF_OUTBOUND))
	{
	  ...
	  if (switch_channel_test_flag (channel, CF_ANSWERED)
	      && !switch_test_flag (member->conference, CFLAG_ANSWERED))
	    {
	      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
	  ...
	  if (member->conference->dtmf_parser != NULL)
	      ...
	      for (digit = dtmf; *digit && caller_action == NULL; digit++)
		{
		  caller_action = (caller_control_action_t *)
		    switch_ivr_digit_stream_parser_feed (member->conference->
							 dtmf_parser,
							 member->digit_stream,
							 *digit);
		}
      ...
      else if (member->conference->dtmf_parser != NULL)
	{
      ...
      if (caller_action != NULL && caller_action->fndesc != NULL
	  && caller_action->fndesc->handler != NULL)
	{
	  char *param = NULL;
      ...
      if (member->fnode)
	{
	  ...
	  if (member->fnode->done)
		...
		{		/* send the node frame instead of the conference frame to the call leg */
		  if (member->fnode->type == NODE_TYPE_SPEECH)
		    {
			...
			SWITCH_SPEECH_FLAG_BLOCKING;
		      ...
		      if (switch_core_speech_read_tts
			  (member->fnode->sh, file_frame, &file_data_len,
			   &rate, &flags) == SWITCH_STATUS_SUCCESS)
			{
			  file_sample_len = file_data_len / 2;
			}
		  ...
		  if (file_sample_len <= 0)
		    ...
		    {		/* there is file node data to deliver */
			...
			{
			  switch_change_sln_volume (write_frame.data,
						    write_frame.samples,
						    member->volume_out_level);
			}
		      write_frame.timestamp = timer.samplecount;
		      switch_core_session_write_frame (member->session,
						       &write_frame,
						       SWITCH_IO_FLAG_NONE,
						       0);
      ...
      else
	{
	  ...
	  uint32_t mux_used =
	    (uint32_t) switch_buffer_inuse (member->mux_buffer);
	  ...
	  if (mux_used)
	    {
	      ...
	      if ((write_frame.datalen =
		   (uint32_t) switch_buffer_read (use_buffer,
						  write_frame.data, bytes)))
		{
		  if (write_frame.datalen
		      && switch_test_flag (member, MFLAG_CAN_HEAR))
		    {
	      ...
	      if (switch_test_flag (member, MFLAG_WASTE_BANDWIDTH))
		{
		  if (member->conference->comfort_noise_level)
		    {
		      switch_generate_sln_silence (write_frame.data, samples,
						   member->conference->
						   comfort_noise_level);
		    }
		  else
  ...
  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
		     "Channel leaving conference, cause: %s\n",
		     switch_channel_cause2str (switch_channel_get_cause
					       (channel)));

  /* if it's an outbound channel, store the release cause in the conference struct, we might need it */
  if (switch_channel_test_flag (channel, CF_OUTBOUND))
    {
      member->conference->bridge_hangup_cause =
...
static void *SWITCH_THREAD_FUNC
conference_record_thread_run (switch_thread_t * thread, void *obj)
{
  ...
  if (switch_thread_rwlock_tryrdlock (conference->rwlock) !=
      SWITCH_STATUS_SUCCESS)
    {
  ...
  switch_mutex_init (&member->audio_out_mutex, SWITCH_MUTEX_NESTED,
		     rec->pool);

  /* Setup an audio buffer for the incoming audio */
  if (switch_buffer_create_dynamic
      (&member->audio_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
       0) != SWITCH_STATUS_SUCCESS)
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
			 "Memory Error Creating Audio Buffer!\n");
      goto end;
    }

  /* Setup an audio buffer for the outgoing audio */
  if (switch_buffer_create_dynamic
      (&member->mux_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
       0) != SWITCH_STATUS_SUCCESS)
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
			 "Memory Error Creating Audio Buffer!\n");
  ...
  if (switch_core_file_open (&fh,
			     rec->path, (uint8_t) 1, conference->rate,
  ...
  while (switch_test_flag (member, MFLAG_RUNNING)
	 && switch_test_flag (conference, CFLAG_RUNNING) && conference->count)
    {
      switch_size_t len;
      ...
      if (mux_used)
	{
	  ...
	  if ((rlen =
	       (uint32_t) switch_buffer_read (member->mux_buffer, data,
					      sizeof (data))))
	    {
      ...
      else
	{
      ...
      if (!switch_test_flag ((&fh), SWITCH_FILE_PAUSE))
	{
	  if (switch_core_file_write (&fh, data, &len) !=
	      SWITCH_STATUS_SUCCESS)
	    {
	      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
				 "Write Failed\n");
...
static uint32_t
conference_stop_file (conference_obj_t * conference, file_stop_t stop)
{
  uint32_t count = 0;
...
static uint32_t
conference_member_stop_file (conference_member_t * member, file_stop_t stop)
{
  conference_file_node_t *nptr;
  uint32_t count = 0;
...
static switch_status_t
conference_play_file (conference_obj_t * conference, char *file,
		      uint32_t leadin, switch_channel_t * channel,
		      uint8_t async)
{
  switch_status_t status = SWITCH_STATUS_SUCCESS;
  ...
  if (channel)
    {
      if ((expanded =
	   switch_channel_expand_variables (channel, file)) != file)
	{
	  file = expanded;
	}
      else
  ...
  if (!switch_is_file_path (file))
      ...
      if (conference->sound_prefix)
	{
	  if (!
	      (dfile =
	       switch_mprintf ("%s%s%s", conference->sound_prefix,
			       SWITCH_PATH_SEPARATOR, file)))
	    {
	      goto done;
	    }
	  file = dfile;
  ...
  if (switch_core_file_open
      (&fnode->fh, file, (uint8_t) 1, conference->rate,
...
static switch_status_t
conference_member_play_file (conference_member_t * member, char *file,
			     uint32_t leadin)
{
  switch_status_t status = SWITCH_STATUS_FALSE;
  char *dfile = NULL, *expanded = NULL;
  ...
  if ((expanded =
       switch_channel_expand_variables (switch_core_session_get_channel
					(member->session), file)) != file)
    {
      file = expanded;
    }
  else
  ...
  if (!switch_is_file_path (file))
      ...
      if (member->conference->sound_prefix)
	{
	  if (!
	      (dfile =
	       switch_mprintf ("%s%s%s", member->conference->sound_prefix,
			       SWITCH_PATH_SEPARATOR, file)))
	    {
	      goto done;
	    }
  ...
  if (switch_core_file_open (&fnode->fh,
			     file, (uint8_t) 1, member->conference->rate,
			     SWITCH_FILE_FLAG_READ | SWITCH_FILE_DATA_SHORT,
			     pool) != SWITCH_STATUS_SUCCESS)
    {
      switch_core_destroy_memory_pool (&pool);
...
static switch_status_t
conference_member_say (conference_member_t * member, char *text,
		       uint32_t leadin)
{
  ...
  if (!member->sh)
    {
      ...
      if (switch_core_speech_open
	  (&member->lsh, conference->tts_engine, conference->tts_voice,
	   conference->rate, conference->interval, &flags,
	   switch_core_session_get_pool (member->session)) !=
	  SWITCH_STATUS_SUCCESS)
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			     "Invalid TTS module [%s]!\n",
  ...
  if (*text == '#')
    ...
    {
      switch_core_speech_text_param_tts (fnode->sh, "voice",
...
/* Say some thing with TTS in the conference room */
static switch_status_t
conference_say (conference_obj_t * conference, const char *text,
		uint32_t leadin)
{
  switch_status_t status = SWITCH_STATUS_FALSE;
  conference_file_node_t *fnode, *nptr;
  ...
  if (!conference->sh)
    {
      ...
      if (switch_core_speech_open
	  (&conference->lsh, conference->tts_engine, conference->tts_voice,
	   conference->rate, conference->interval, &flags,
	   conference->pool) != SWITCH_STATUS_SUCCESS)
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
...
static void
conference_member_itterator (conference_obj_t * conference,
			     switch_stream_handle_t * stream,
...
static void
conference_list_pretty (conference_obj_t * conference,
			switch_stream_handle_t * stream)
{
  conference_member_t *member = NULL;
...
static void
conference_list (conference_obj_t * conference,
		 switch_stream_handle_t * stream, char *delim)
{
  conference_member_t *member = NULL;
  ...
  for (member = conference->members; member; member = member->next)
      ...
      if (switch_test_flag (member, MFLAG_NOCHANNEL))
	{
      ...
      stream->write_function (stream, "%u%s%s%s%s%s%s%s%s%s",
			      member->id, delim, name, delim, uuid, delim,
      ...
      if (switch_test_flag (member, MFLAG_CAN_SPEAK))
	{
      ...
      if (switch_test_flag (member, MFLAG_TALKING))
	{
	  stream->write_function (stream, "%s%s", count ? "|" : "",
				  "talking");
	  count++;
	}

      if (switch_channel_test_flag
	  (switch_core_session_get_channel (member->session), CF_VIDEO))
	{
...
static switch_status_t
conf_api_sub_mute (conference_member_t * member,
		   switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (!switch_strlen_zero (member->conference->muted_sound))
    {
      conference_member_play_file (member, member->conference->muted_sound,
				   0);
    }
  else
    {
...
static switch_status_t
conf_api_sub_unmute (conference_member_t * member,
		     switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (!switch_strlen_zero (member->conference->unmuted_sound))
    {
      conference_member_play_file (member, member->conference->unmuted_sound,
				   0);
    }
  else
    {
...
static switch_status_t
conf_api_sub_deaf (conference_member_t * member,
		   switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (switch_event_create_subclass
      (&event, SWITCH_EVENT_CUSTOM,
       CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS)
    {
      conference_add_event_member_data (member, event);
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "deaf-member");
...
static switch_status_t
conf_api_sub_undeaf (conference_member_t * member,
		     switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
...
static switch_status_t
conf_api_sub_kick (conference_member_t * member,
		   switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (member->conference
      && test_eflag (member->conference, EFLAG_KICK_MEMBER))
    {
      if (switch_event_create_subclass
	  (&event, SWITCH_EVENT_CUSTOM,
	   CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS)
...
static switch_status_t
conf_api_sub_dtmf (conference_member_t * member,
		   switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  char *dtmf = (char *) data;
...
static switch_status_t
conf_api_sub_energy (conference_member_t * member,
		     switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (test_eflag (member->conference, EFLAG_ENERGY_LEVEL_MEMBER) &&
      ...
      SWITCH_STATUS_SUCCESS)
    {
      ...
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "energy-level-member");
      switch_event_add_header (event, SWITCH_STACK_BOTTOM, "Energy-Level",
			       "%d", member->energy_level);
      switch_event_fire (&event);
    }
...
static switch_status_t
conf_api_sub_volume_in (conference_member_t * member,
			switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
  ...
  if (test_eflag (member->conference, EFLAG_VOLUME_IN_MEMBER) &&
      ...
      SWITCH_STATUS_SUCCESS)
    {
      ...
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "volume-in-member");
      switch_event_add_header (event, SWITCH_STACK_BOTTOM, "Volume-Level",
			       "%u", member->volume_in_level);
      switch_event_fire (&event);
    }
...
static switch_status_t
conf_api_sub_volume_out (conference_member_t * member,
			 switch_stream_handle_t * stream, void *data)
{
  switch_event_t *event;
...
static switch_status_t
conf_api_sub_list (conference_obj_t * conference,
...
static switch_status_t
conf_api_sub_play (conference_obj_t * conference,
		   switch_stream_handle_t * stream, int argc, char **argv)
{
  int ret_status = SWITCH_STATUS_GENERR;
  switch_event_t *event;
  ...
  if ((argc == 4 && !strcasecmp (argv[3], "async"))
      || (argc == 5 && !strcasecmp (argv[4], "async")))
    {
      argc--;
      async++;
    }
  ...
  if (argc == 3)
    {
      if (conference_play_file (conference, argv[2], 0, NULL, async) ==
	  SWITCH_STATUS_SUCCESS)
	{
	...
	{
	  if (conference_member_play_file (member, argv[2], 0) ==
	      SWITCH_STATUS_SUCCESS)
	    {
...
static switch_status_t
conf_api_sub_say (conference_obj_t * conference,
		  switch_stream_handle_t * stream, const char *text)
{
  switch_event_t *event;
  ...
  if (test_eflag (conference, EFLAG_SPEAK_TEXT)
      ...
      SWITCH_STATUS_SUCCESS)
    {
      ...
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Action",
				      "speak-text");
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "Text",
				      text);
      switch_event_fire (&event);
    }
  return SWITCH_STATUS_SUCCESS;
...
static switch_status_t
conf_api_sub_saymember (conference_obj_t * conference,
			switch_stream_handle_t * stream, const char *text)
{
  int ret_status = SWITCH_STATUS_GENERR;
  char *expanded = NULL;
  ...
  if (!text
      || conference_member_say (member, (char *) text,
				0) != SWITCH_STATUS_SUCCESS)
    {
...
static switch_status_t
conf_api_sub_stop (conference_obj_t * conference,
		   switch_stream_handle_t * stream, int argc, char **argv)
{
  ...
  if (argc == 4)
	...
	{
	  uint32_t stopped =
	    conference_member_stop_file (member,
					 async ? FILE_STOP_ASYNC : current ?
					 FILE_STOP_CURRENT : FILE_STOP_ALL);
    ...
    {
      uint32_t stopped =
	conference_stop_file (conference,
			      async ? FILE_STOP_ASYNC : current ?
			      FILE_STOP_CURRENT : FILE_STOP_ALL);
      stream->write_function (stream, "Stopped %u files.\n", stopped);
    }
...
static switch_status_t
conf_api_sub_relate (conference_obj_t * conference,
		     switch_stream_handle_t * stream, int argc, char **argv)
{
...
static switch_status_t
conf_api_sub_lock (conference_obj_t * conference,
		   switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_event_t *event;
  ...
  if (conference->is_locked_sound)
    {
      conference_play_file (conference, conference->is_locked_sound,
			    CONF_DEFAULT_LEADIN, NULL, 0);
    }
...
static switch_status_t
conf_api_sub_unlock (conference_obj_t * conference,
		     switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_event_t *event;
  ...
  if (conference->is_unlocked_sound)
    {
      conference_play_file (conference, conference->is_unlocked_sound,
			    CONF_DEFAULT_LEADIN, NULL, 0);
    }
...
static switch_status_t
conf_api_sub_dial (conference_obj_t * conference,
		   switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_call_cause_t cause;
  ...
  if (conference)
    {
      conference_outcall (conference, NULL, NULL, argv[2], 60, NULL, argv[4],
			  argv[3], &cause);
    }
  else
    {
      conference_outcall (NULL, argv[0], NULL, argv[2], 60, NULL, argv[4],
			  argv[3], &cause);
    }
...
static switch_status_t
conf_api_sub_bgdial (conference_obj_t * conference,
		     switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_assert (stream != NULL);
  ...
  if (conference)
    {
      conference_outcall_bg (conference, NULL, NULL, argv[2], 60, NULL,
			     argv[4], argv[3]);
    }
  else
    {
      conference_outcall_bg (NULL, argv[1], NULL, argv[2], 60, NULL, argv[4],
			     argv[3]);
    }
...
static switch_status_t
conf_api_sub_transfer (conference_obj_t * conference,
		       switch_stream_handle_t * stream, int argc, char **argv)
{
  ...
  if (argc > 3 && !switch_strlen_zero (argv[2]))
      ...
      for (x = 3; x < argc; x++)
	{
	  ...
	  if (!
	      (new_conference =
	       (conference_obj_t *) switch_core_hash_find (globals.
							   conference_hash,
							   conf_name)))
	    {
	      ...
	      /* Setup a memory pool to use. */
	      if (switch_core_new_memory_pool (&pool) !=
		  SWITCH_STATUS_SUCCESS)
		{
		  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
				     "Pool Failure\n");
	      ...
	      switch_event_add_header_string (params, SWITCH_STACK_BOTTOM,
					      "conf_name", conf_name);
	      switch_event_add_header_string (params, SWITCH_STACK_BOTTOM,
	      ...
	      if (!
		  (cxml = switch_xml_open_cfg (global_cf_name, &cfg, params)))
		{
		  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
				     "Open of %s failed\n", global_cf_name);
		  goto done;
		...
		{
		  xml_cfg.profile =
		    switch_xml_find_child (profiles, "profile", "name",
					   profile_name);
		}
	  ...
	  switch_mutex_unlock (member->flag_mutex);
	  stream->write_function (stream,
				  "OK Members sent to conference %s.\n",
				  argv[2]);
...
static switch_status_t
conf_api_sub_record (conference_obj_t * conference,
		     switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_assert (conference != NULL);
  switch_assert (stream != NULL);
...
static switch_status_t
conf_api_sub_norecord (conference_obj_t * conference,
		       switch_stream_handle_t * stream, int argc, char **argv)
{
  int all;
...
static switch_status_t
conf_api_sub_pin (conference_obj_t * conference,
		  switch_stream_handle_t * stream, int argc, char **argv)
{
  switch_assert (conference != NULL);
  switch_assert (stream != NULL);
...
static api_command_t conf_api_sub_commands[] = {
  {"list", (void_fn_t) & conf_api_sub_list, CONF_API_SUB_ARGS_SPLIT,
  ...
  {"volume_in", (void_fn_t) & conf_api_sub_volume_in,
   CONF_API_SUB_MEMBER_TARGET,
   "<confname> volume_in <member_id|all|last> [<newval>]"},
  {"volume_out", (void_fn_t) & conf_api_sub_volume_out,
   CONF_API_SUB_MEMBER_TARGET,
   "<confname> volume_out <member_id|all|last> [<newval>]"},
  {"play", (void_fn_t) & conf_api_sub_play, CONF_API_SUB_ARGS_SPLIT,
   "<confname> play <file_path> [async|<member_id>]"},
  {"say", (void_fn_t) & conf_api_sub_say, CONF_API_SUB_ARGS_AS_ONE,
   "<confname> say <text>"},
  {"saymember", (void_fn_t) & conf_api_sub_saymember,
   CONF_API_SUB_ARGS_AS_ONE,
  ...
  {"mute", (void_fn_t) & conf_api_sub_mute, CONF_API_SUB_MEMBER_TARGET,
  ...
  {"deaf", (void_fn_t) & conf_api_sub_deaf, CONF_API_SUB_MEMBER_TARGET,
  ...
  {"lock", (void_fn_t) & conf_api_sub_lock, CONF_API_SUB_ARGS_SPLIT,
   "<confname> lock"},
  {"unlock", (void_fn_t) & conf_api_sub_unlock, CONF_API_SUB_ARGS_SPLIT,
   "<confname> unlock"},
  ...
  {"record", (void_fn_t) & conf_api_sub_record, CONF_API_SUB_ARGS_SPLIT,
   "<confname> record <filename>"},
  {"norecord", (void_fn_t) & conf_api_sub_norecord, CONF_API_SUB_ARGS_SPLIT,
   "<confname> norecord <[filename|all]>"},
  {"pin", (void_fn_t) & conf_api_sub_pin, CONF_API_SUB_ARGS_SPLIT,
   "<confname> pin <pin#>"},
  {"nopin", (void_fn_t) & conf_api_sub_pin, CONF_API_SUB_ARGS_SPLIT,
   "<confname> nopin"},
};
...
switch_status_t
conf_api_dispatch (conference_obj_t * conference,
		   switch_stream_handle_t * stream, int argc, char **argv,
      ...
      if (strcasecmp (argv[argn], conf_api_sub_commands[i].pname) == 0)
	{
	  ...
	  switch (conf_api_sub_commands[i].fntype)
	    ...
	    case CONF_API_SUB_ARGS_SPLIT:
	      {
		conf_api_args_cmd_t pfn =
		  (conf_api_args_cmd_t) conf_api_sub_commands[i].pfnapicmd;

		if (pfn (conference, stream, argc, argv) !=
		    SWITCH_STATUS_SUCCESS)
		  {
		    /* command returned error, so show syntax usage */
		    stream->write_function (stream,
					    conf_api_sub_commands[i].psyntax);
		  }
	      }
	      break;
		...
		if (all)
		  {
		    conference_member_itterator (conference, stream,
						 (conf_api_member_cmd_t)
						 conf_api_sub_commands[i].
						 pfnapicmd, argv[argn + 2]);
		  }
		else if (last)
		  {
		    conference_member_t *member = NULL;
		    ...
		    if (last_member != NULL)
		      {
			conf_api_member_cmd_t pfn =
			  (conf_api_member_cmd_t) conf_api_sub_commands[i].
			  pfnapicmd;
		...
		else if (id)
		  {
		    conf_api_member_cmd_t pfn =
		      ...
		      pfnapicmd;
		    conference_member_t *member =
		      conference_member_get (conference, id);
	    ...
	    case CONF_API_SUB_ARGS_AS_ONE:
	      {
		conf_api_text_cmd_t pfn =
		  (conf_api_text_cmd_t) conf_api_sub_commands[i].pfnapicmd;
		char *start_text;
		const char *modified_cmdline = cmdline;
		const char *cmd = conf_api_sub_commands[i].pname;

		if (!switch_strlen_zero (modified_cmdline)
		    && (start_text = strstr (modified_cmdline, cmd)))
		  {
		    modified_cmdline = start_text + strlen (cmd);
		    while (modified_cmdline
			   && (*modified_cmdline == ' '
			       || *modified_cmdline == '\t'))
		      {
			modified_cmdline++;
		...
		if (pfn (conference, stream, modified_cmdline) !=
		    SWITCH_STATUS_SUCCESS)
		  {
		    /* command returned error, so show syntax usage */
		    stream->write_function (stream,
...
SWITCH_STANDARD_API (conf_api_main)
{
  ...
  argc =
    switch_separate_string (lbuf, ' ', argv,
  ...
  if (argc && argv[0])
						       ...
						       argv[0])))
	{
	  ...
	  if (argc >= 2)
	    ...
	    {
	      stream->write_function (stream,
				      "Conference command, not specified.\nTry 'help'\n");
	    }
	  switch_thread_rwlock_unlock (conference->rwlock);
	  ...
	  if (strcasecmp (argv[0], "list") == 0)
	    {
	      conf_api_sub_list (NULL, stream, argc, argv);
		   ...
		   || strcasecmp (argv[0], "commands") == 0)
	    {
	      stream->write_function (stream, "%s\n", api_syntax);
	    }
	  else if (argv[1] && strcasecmp (argv[1], "dial") == 0)
	    {
	      if (conf_api_sub_dial (NULL, stream, argc, argv) !=
		  SWITCH_STATUS_SUCCESS)
		{
		  /* command returned error, so show syntax usage */
		  stream->write_function (stream,
					  conf_api_sub_commands
					  [CONF_API_COMMAND_DIAL].psyntax);
		}
	    }
	  else if (strcasecmp (argv[0], "bgdial") == 0)
	    {
	      if (conf_api_sub_bgdial (NULL, stream, argc, argv) !=
		  SWITCH_STATUS_SUCCESS)
		{
		  /* command returned error, so show syntax usage */
		  stream->write_function (stream,
					  conf_api_sub_commands
					  [CONF_API_COMMAND_BGDIAL].psyntax);
		}
	    }
	  else
	    {
...
static switch_status_t
conference_outcall (conference_obj_t * conference,
		    char *conference_name,
		    switch_core_session_t * session,
		    char *bridgeto, uint32_t timeout, char *flags,
		    char *cid_name, char *cid_num,
		    switch_call_cause_t * cause)
{
  ...
  *cause = SWITCH_CAUSE_NORMAL_CLEARING;
  ...
  if (conference == NULL)
	...
	switch_mprintf ("{ignore_early_media=true}%s", bridgeto);
      status =
	switch_ivr_originate (NULL, &peer_session, cause, dialstr, 60, NULL,
			      cid_name, cid_num, NULL, SOF_NONE);
  ...
  if (switch_ivr_originate
      (session, &peer_session, cause, bridgeto, timeout, NULL, cid_name,
       cid_num, NULL, SOF_NONE) != SWITCH_STATUS_SUCCESS)
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
  ...
  if (!switch_test_flag (conference, CFLAG_RUNNING))
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			 "Conference is gone now, nevermind..\n");
      if (caller_channel)
	{
...
callup:
  ...
  if (switch_channel_test_flag (peer_channel, CF_ANSWERED)
      || switch_channel_test_flag (peer_channel, CF_EARLY_MEDIA))
    {
      ...
      if ((extension =
	   switch_caller_extension_new (peer_session, conference_name,
					conference_name)) == 0)
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
			     "Memory Error!\n");
	...
	{
	  switch_snprintf (appdata, sizeof (appdata), "%s+flags{%s}",
			   conference_name, flags);
	  switch_caller_extension_add_application (peer_session, extension,
...
static void *SWITCH_THREAD_FUNC
conference_outcall_run (switch_thread_t * thread, void *obj)
{
  ...
  if (call)
    {
      ...
      conference_outcall (call->conference, call->conference_name,
			  ...
			  call->flags, call->cid_name, call->cid_num, &cause);
...
static switch_status_t
conference_outcall_bg (conference_obj_t * conference,
		       char *conference_name,
		       switch_core_session_t * session, char *bridgeto,
		       uint32_t timeout, const char *flags,
		       const char *cid_name, const char *cid_num)
{
  ...
  switch_threadattr_detach_set (thd_attr, 1);
  switch_threadattr_stacksize_set (thd_attr, SWITCH_THREAD_STACKSIZE);
  switch_thread_create (&thread, thd_attr, conference_outcall_run, call,
			conference->pool);
  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
		     "Launching BG Thread for outcall\n");
...
/* Play a file */
static switch_status_t
conference_local_play_file (conference_obj_t * conference,
			    switch_core_session_t * session, char *path,
			    uint32_t leadin, void *buf, uint32_t buflen)
{
  uint32_t x = 0;
  switch_status_t status = SWITCH_STATUS_SUCCESS;
  switch_channel_t *channel;
  ...
  for (x = 0; x < leadin; x++)
    {
      ...
      status =
	switch_core_session_read_frame (session, &read_frame,
					SWITCH_IO_FLAG_NONE, 0);
  ...
  if (status == SWITCH_STATUS_SUCCESS)
    {
      ...
      if ((expanded =
	   switch_channel_expand_variables (channel, path)) != path)
	{
	  path = expanded;
	}
      else
      ...
      if (!strncasecmp (path, "say:", 4))
	{
	  if (!(conference->tts_engine && conference->tts_voice))
	    ...
	    {
	      status =
		switch_ivr_speak_text (session, conference->tts_engine,
				       conference->tts_voice, path + 4, ap);
	    }
	  goto done;
	}
...
static void
set_mflags (char *flags, member_flag_t * f)
{
  if (flags)
    {
...
static void
clear_eflags (char *events, uint32_t * f)
{
  char buf[512] = "";
  char *next = NULL;
  char *event = buf;
...
SWITCH_STANDARD_APP (conference_function)
{
  ...
  switch_assert (params);
  switch_event_add_header_string (params, SWITCH_STACK_BOTTOM, "conf_name",
				  conf_name);
  switch_event_add_header_string (params, SWITCH_STACK_BOTTOM, "profile_name",
				  profile_name);
  ...
  if ((profiles = switch_xml_child (cfg, "profiles")))
    {
      xml_cfg.profile =
	switch_xml_find_child (profiles, "profile", "name", profile_name);
    }
  ...
  if (isbr)
      ...
      if (!conference)
	{
      ...
      switch_channel_set_variable (channel, "conference_name",
				   conference->name);
	  ...
	  && *(conference->pin))
	{
	  ...
	  while (!pin_valid && pin_retries && status == SWITCH_STATUS_SUCCESS)
		...
		{
		  conference_local_play_file (conference, session,
					      conference->pin_sound, 20,
	      ...
	      if (strlen (pin_buf) < strlen (conference->pin))
		{
	      ...
	      pin_valid = (status == SWITCH_STATUS_SUCCESS
			   && strcmp (pin_buf, conference->pin) == 0);
	      if (!pin_valid)
		{
	  ...
	  && !switch_channel_test_flag (channel, CF_OUTBOUND))
	{
	  ...
	  if (conference->locked_sound)
	    {
	      ...
	      switch_channel_answer (channel);
	      conference_local_play_file (conference, session,
					  conference->locked_sound, 20, NULL,
					  0);
	    }
	  goto done;
	}
	  ...
	  && (conference->count >= conference->max_members))
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
			     "Conference %s is full.\n", conf_name);
	  if (conference->maxmember_sound)
	    {
	      /* Answer the channel */
	      switch_channel_answer (channel);
	      conference_local_play_file (conference, session,
					  conference->maxmember_sound, 20,
					  NULL, 0);
	    }
	  goto done;
	}
  ...
  if (!switch_strlen_zero (bridgeto) && strcasecmp (bridgeto, "none"))
    {
      switch_call_cause_t cause;
      if (conference_outcall
	  (conference, NULL, session, bridgeto, 60, NULL, NULL, NULL,
	   &cause) != SWITCH_STATUS_SUCCESS)
	{
	  goto done;
	}
    }
  ...
  if (switch_core_codec_init (&member.read_codec,
			      ...
			      read_codec->implementation->
			      microseconds_per_packet / 1000, 1,
			      SWITCH_CODEC_FLAG_ENCODE |
			      SWITCH_CODEC_FLAG_DECODE, NULL,
    ...
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
			 "Raw Codec Activation Failed L16@%uhz 1 channel %dms\n",
			 read_codec->implementation->
			 actual_samples_per_second,
			 read_codec->implementation->microseconds_per_packet /
			 1000);
  ...
  if (read_codec->implementation->actual_samples_per_second !=
      conference->rate)
    {
      if (switch_resample_create (&member.read_resampler,
				  read_codec->implementation->
				  actual_samples_per_second,
				  member.frame_size, conference->rate,
				  member.frame_size,
				  member.pool) != SWITCH_STATUS_SUCCESS)
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
			     "Unable to create resampler!\n");
	  goto done;
	}


      member.resample_out =
	switch_core_alloc (member.pool, member.frame_size);
      member.resample_out_len = member.frame_size;

      /* Setup an audio buffer for the resampled audio */
      if (switch_buffer_create_dynamic
	  (&member.resample_buffer, CONF_DBLOCK_SIZE, CONF_DBUFFER_SIZE,
	   CONF_DBUFFER_MAX) != SWITCH_STATUS_SUCCESS)
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT,
			     "Memory Error Creating Audio Buffer!\n");
	  goto done;
  ...
  switch_mutex_init (&member.audio_in_mutex, SWITCH_MUTEX_NESTED,
		     member.pool);
  switch_mutex_init (&member.audio_out_mutex, SWITCH_MUTEX_NESTED,
		     member.pool);
...
codec_done2:
  ...
  if (conference && switch_test_flag (&member, MFLAG_KICKED)
      && conference->kicked_sound)
    {
      char *toplay = NULL;
      char *dfile = NULL;
      ...
      if (conference->sound_prefix)
	{
	  dfile =
	    switch_mprintf ("%s%s%s", conference->sound_prefix,
			    SWITCH_PATH_SEPARATOR, conference->kicked_sound);
	  switch_assert (dfile);
	  toplay = dfile;
	}
      else
...
static void
launch_conference_thread (conference_obj_t * conference)
{
  switch_thread_t *thread;
  switch_threadattr_t *thd_attr = NULL;
  ...
  switch_core_hash_insert (globals.conference_hash, conference->name,
			   conference);
  switch_mutex_unlock (globals.hash_mutex);
  switch_thread_create (&thread, thd_attr, conference_thread_run, conference,
			conference->pool);
}

static void
launch_conference_record_thread (conference_obj_t * conference, char *path)
{
  switch_thread_t *thread;
  switch_threadattr_t *thd_attr = NULL;
  switch_memory_pool_t *pool;
  ...
  switch_threadattr_detach_set (thd_attr, 1);
  switch_threadattr_stacksize_set (thd_attr, SWITCH_THREAD_STACKSIZE);
  switch_thread_create (&thread, thd_attr, conference_record_thread_run, rec,
			rec->pool);
}

static switch_status_t
chat_send (char *proto, char *from, char *to, char *subject, char *body,
	   char *hint)
{
  ...
  if (!
      (conference =
       (conference_obj_t *) switch_core_hash_find (globals.conference_hash,
						   name)))
    {
      ci->chat_send (CONF_CHAT_PROTO, to, hint
		     && strchr (hint, '/') ? hint : from, "",
		     "Conference not active.", NULL);
      return SWITCH_STATUS_FALSE;
    }
  ...
  if (body != NULL && (lbuf = strdup (body)))
	...
	{
	  conf_api_dispatch (conference, &stream, argc, argv,
			     (const char *) body, 0);
	}
  ...
  ci->chat_send (CONF_CHAT_PROTO, to, hint
		 && strchr (hint, '/') ? hint : from, "", stream.data, NULL);
...
static switch_status_t
conf_default_controls (conference_obj_t * conference)
{
  switch_status_t status = SWITCH_STATUS_FALSE;
  uint32_t i;
  caller_control_action_t *action;
  ...
  for (i = 0, status = SWITCH_STATUS_SUCCESS;
       status == SWITCH_STATUS_SUCCESS && i < CCFNTBL_QTY; i++)
    {
	  ...
	  if (action != NULL)
	    {
	      ...
	      action->data = NULL;
	      status =
		switch_ivr_digit_stream_parser_set_event (conference->
							  dtmf_parser,
							  ccfntbl[i].digits,
							  action);
	    }
...
static switch_status_t
conference_new_install_caller_controls_custom (conference_obj_t * conference,
					       ...
					       switch_xml_t xml_menus)
{
  ...
  for (xml_kvp = switch_xml_child (xml_controls, "control"); xml_kvp;
      ...
      if (!switch_strlen_zero (key) && !switch_strlen_zero (val))
	{
	  ...
	  for (i = 0, status = SWITCH_STATUS_NOOP;
	       i < CCFNTBL_QTY && status == SWITCH_STATUS_NOOP; i++)
	    {
		  ...
		  if (action != NULL)
		    {
		      action->fndesc = &ccfntbl[i];
		      action->data =
			(void *) switch_core_strdup (conference->pool, data);
		      action->binded_dtmf =
			switch_core_strdup (conference->pool, val);
		      status =
			switch_ivr_digit_stream_parser_set_event (conference->
								  dtmf_parser,
								  val,
	  ...
	  if (status == SWITCH_STATUS_NOOP)
	    {
	      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
				 "Invalid caller control action name '%s'.\n",
				 key);
	    }
	}
      else
...
static conference_obj_t *
conference_new (char *name, conf_xml_cfg_t cfg, switch_memory_pool_t * pool)
{
  conference_obj_t *conference;
  switch_xml_t xml_kvp;
  ...
  for (xml_kvp = switch_xml_child (cfg.profile, "param"); xml_kvp;
       xml_kvp = xml_kvp->next)
    {
	       ...
	       && !switch_strlen_zero (val))
	{
      ...
      else if (!strcasecmp (var, "comfort-noise")
	       && !switch_strlen_zero (val))
	{
	  int tmp;
	  tmp = atoi (val);
      ...
      else if (!strcasecmp (var, "max-members") && !switch_strlen_zero (val))
	{
	  ...
	  if (errno == ERANGE || errno == EINVAL || max_members < 0
	      || max_members == 1)
	    {
	       ...
	       && !switch_strlen_zero (val))
	{
      ...
      else if (!strcasecmp (var, "announce-count")
	       && !switch_strlen_zero (val))
	{
	  ...
	  if (errno == ERANGE || errno == EINVAL)
	    {
	      announce_count = 0;
	      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
				 "announce-count is invalid, not anouncing member counts\n");
	    }
	}
  ...
  conference->profile_name =
    switch_core_strdup (conference->pool,
			switch_xml_attr_soft (cfg.profile, "name"));
  if (timer_name)
    {
      conference->timer_name =
	switch_core_strdup (conference->pool, timer_name);
    }
  if (tts_engine)
    {
      conference->tts_engine =
	switch_core_strdup (conference->pool, tts_engine);
    }
  if (tts_voice)
    {
      conference->tts_voice =
	switch_core_strdup (conference->pool, tts_voice);
    }

  conference->comfort_noise_level = comfort_noise_level;
  conference->caller_id_name =
    switch_core_strdup (conference->pool, caller_id_name);
  conference->caller_id_number =
    switch_core_strdup (conference->pool, caller_id_number);


  if (!switch_strlen_zero (perpetual_sound))
    {
      conference->perpetual_sound =
	switch_core_strdup (conference->pool, perpetual_sound);
    }
  ...
  if (!switch_strlen_zero (moh_sound) && switch_is_moh (moh_sound))
    {
      conference->moh_sound =
	switch_core_strdup (conference->pool, moh_sound);
    }
  ...
  if (sound_prefix)
	...
	switch_core_strdup (conference->pool, sound_prefix);
    }

  if (!switch_strlen_zero (enter_sound))
    {
      conference->enter_sound =
	switch_core_strdup (conference->pool, enter_sound);
    }

  if (!switch_strlen_zero (exit_sound))
    {
      conference->exit_sound =
	switch_core_strdup (conference->pool, exit_sound);
    }

  if (!switch_strlen_zero (ack_sound))
    {
      conference->ack_sound =
	switch_core_strdup (conference->pool, ack_sound);
    }

  if (!switch_strlen_zero (nack_sound))
    {
      conference->nack_sound =
	switch_core_strdup (conference->pool, nack_sound);
    }

  if (!switch_strlen_zero (muted_sound))
    {
      conference->muted_sound =
	switch_core_strdup (conference->pool, muted_sound);
    }

  if (!switch_strlen_zero (unmuted_sound))
    {
      conference->unmuted_sound =
	switch_core_strdup (conference->pool, unmuted_sound);
    }

  if (!switch_strlen_zero (kicked_sound))
    {
      conference->kicked_sound =
	switch_core_strdup (conference->pool, kicked_sound);
    }

  if (!switch_strlen_zero (pin_sound))
    {
      conference->pin_sound =
	switch_core_strdup (conference->pool, pin_sound);
    }

  if (!switch_strlen_zero (bad_pin_sound))
    {
      conference->bad_pin_sound =
	switch_core_strdup (conference->pool, bad_pin_sound);
    }
  ...
  if (!switch_strlen_zero (alone_sound))
    {
      conference->alone_sound =
	switch_core_strdup (conference->pool, alone_sound);
    }

  if (!switch_strlen_zero (locked_sound))
    {
      conference->locked_sound =
	switch_core_strdup (conference->pool, locked_sound);
    }

  if (!switch_strlen_zero (is_locked_sound))
    {
      conference->is_locked_sound =
	switch_core_strdup (conference->pool, is_locked_sound);
    }

  if (!switch_strlen_zero (is_unlocked_sound))
    {
      conference->is_unlocked_sound =
	switch_core_strdup (conference->pool, is_unlocked_sound);
    }
  ...
  if (!switch_strlen_zero (maxmember_sound))
    {
      conference->maxmember_sound =
	switch_core_strdup (conference->pool, maxmember_sound);
    }
  ...
  if (switch_ivr_digit_stream_parser_new
      (conference->pool, &conference->dtmf_parser) == SWITCH_STATUS_SUCCESS)
    {

      /* if no controls, or default controls specified, install default */
      if (caller_controls == NULL || *caller_controls == '\0'
	  || strcasecmp (caller_controls, "default") == 0)
	{
      ...
      else if (strcasecmp (caller_controls, "none") != 0)
	{
	  ...
	  switch_xml_t xml_controls =
	    switch_xml_find_child (cfg.controls, "group", "name",
				   caller_controls);
	  status =
	    conference_new_install_caller_controls_custom (conference,
							   xml_controls,
							   NULL);
      ...
      else
	{
	  switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
			     "No caller controls installed.\n");
	}
    }
  else
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			 "Unable to allocate caller control digit parser.\n");
    }

  /* Activate the conference mutex for exclusivity */
  switch_mutex_init (&conference->mutex, SWITCH_MUTEX_NESTED,
		     conference->pool);
  switch_mutex_init (&conference->flag_mutex, SWITCH_MUTEX_NESTED,
		     conference->pool);
  switch_thread_rwlock_create (&conference->rwlock, conference->pool);
  switch_mutex_init (&conference->member_mutex, SWITCH_MUTEX_NESTED,
		     conference->pool);
...
static void
pres_event_handler (switch_event_t * event)
{
  char *to = switch_event_get_header (event, "to");
  ...
  else if (switch_event_create (&event, SWITCH_EVENT_PRESENCE_IN) ==
	   SWITCH_STATUS_SUCCESS)
    {
      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM, "proto",
				      CONF_CHAT_PROTO);
...
static void
send_presence (switch_event_types_t id)
{
  ...
  switch_event_create (&params, SWITCH_EVENT_COMMAND);
  switch_assert (params);
  switch_event_add_header_string (params, SWITCH_STACK_BOTTOM, "presence",
				  "true");
  ...
  if ((advertise = switch_xml_child (cfg, "advertise")))
    {
      for (room = switch_xml_child (advertise, "room"); room;
	   room = room->next)
	{
	  ...
	  if (name
	      && switch_event_create (&event, id) == SWITCH_STATUS_SUCCESS)
	    {
	      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					      "proto", CONF_CHAT_PROTO);
	      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					      "login", name);
	      switch_event_add_header_string (event, SWITCH_STACK_BOTTOM,
					      "from", name);
...
SWITCH_MODULE_LOAD_FUNCTION (mod_conference_load)
{
  ...
  *module_interface =
  ...
  if (switch_event_reserve_subclass (CONF_EVENT_MAINT) !=
      SWITCH_STATUS_SUCCESS)
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
  ...
  switch_mutex_init (&globals.conference_mutex, SWITCH_MUTEX_NESTED,
		     globals.conference_pool);
  switch_mutex_init (&globals.id_mutex, SWITCH_MUTEX_NESTED,
		     globals.conference_pool);
  switch_mutex_init (&globals.hash_mutex, SWITCH_MUTEX_NESTED,
		     globals.conference_pool);
  ...
  if (switch_event_bind_removable
    ...
    {
      switch_log_printf (SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			 "Couldn't subscribe to presence request events!\n");
      return SWITCH_STATUS_GENERR;
    }

  SWITCH_ADD_API (api_interface, "conference", "Conference module commands",
		  conf_api_main, p);
  SWITCH_ADD_APP (app_interface, global_app_name, global_app_name, NULL,
		  conference_function, NULL, SAF_NONE);
  SWITCH_ADD_APP (app_interface, "conference_set_auto_outcall",
		  "conference_set_auto_outcall", NULL,
