													...
													switch_call_cause_t *cancel_cause)
{
	...
	if ((*new_session = switch_core_session_request_uuid(pulseaudio_endpoint_interface, SWITCH_CALL_DIRECTION_OUTBOUND, flags, pool, switch_event_get_header(var_event, "origination_uuid"))) != 0) {
		private_t *tech_pvt;
		switch_channel_t *channel;
		switch_caller_profile_t *caller_profile;
		...
		if (outbound_profile) {
			...
			snprintf(name, sizeof(name), "pulseaudio/%s", id);
...
static switch_status_t place_call(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if ((session = switch_core_session_request(pulseaudio_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE, NULL)) != 0) {
		...
		if ((tech_pvt->caller_profile = switch_caller_profile_new(switch_core_session_get_pool(session),
			...
			char name[128];
			snprintf(name, sizeof(name), "pulseaudio/%s", tech_pvt->caller_profile->destination_number ? tech_pvt->caller_profile->destination_number : modname);
			switch_channel_set_name(channel, name);
...
SWITCH_STANDARD_API(pa_cmd)
{
	...
	const char *usage_string = "USAGE:\n"
		"--------------------------------------------------------------------------------\n"
		"pulseaudio help\n"
		"pulseaudio call <dest> [<dialplan> <cid_name> <cid_num> <rate>]\n"
		"pulseaudio answer [<call_id>]\n"
		"pulseaudio hangup [<call_id>]\n"
		"pulseaudio list\n"
