	...
	*new_session = NULL;
	...
	if (!strncasecmp(profile_name, "gateway", 7)) {
		...
		*dest++ = '\0';
		...
		if (!strchr(dest, '@')) {
			tech_pvt->dest = switch_core_session_sprintf(nsession, "sip:%s@%s", dest, 
														 gateway_ptr->outbound_sticky_proxy ? 
														 sofia_glue_strip_proto(gateway_ptr->outbound_sticky_proxy) :
