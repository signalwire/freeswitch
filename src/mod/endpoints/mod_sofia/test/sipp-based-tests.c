...
FST_CORE_EX_BEGIN("./conf-sipp", SCF_VG | SCF_USE_SQL)
{
	FST_MODULE_BEGIN(mod_sofia, uac-uas)
	{
		...
		FST_TEST_BEGIN(uac_telephone_event_check)
		{
			...
			sipp_ret = start_sipp_uac(local_ip_v4, 5084, "1212121212", "sipp-scenarios/uac_telephone_event.xml", "");
		...
		FST_TEST_BEGIN(uac_savp_check)
		{
			...
			sipp_ret = start_sipp_uac(local_ip_v4, 5084, "1212121212", "sipp-scenarios/uac_savp_check.xml", "");
			if (sipp_ret < 0 || sipp_ret == 127) {
				fst_check(!"sipp not found");
			} else {
				test_wait_for_uuid(uuid);
		...
		FST_TEST_BEGIN(uac_digest_leak_udp)
		{
			...
			sipp_ret = start_sipp_uac(local_ip_v4, 5084, "1001", "sipp-scenarios/uac_digest_leak.xml", "");
			if (sipp_ret < 0 || sipp_ret == 127) {
				fst_check(!"sipp not found");
			} else {
		...
		FST_TEST_BEGIN(uac_digest_leak_tcp)
		{
			...
			status = switch_ivr_originate(NULL, &session, &cause, "loopback/+15553334444", timeout_sec, NULL, NULL, NULL, NULL, NULL, SOF_NONE, NULL, NULL);
			sipp_ret = start_sipp_uac(local_ip_v4, 5084, "1001", "sipp-scenarios/uac_digest_leak-tcp.xml", "-t t1");
			if (sipp_ret < 0 || sipp_ret == 127) {
				fst_check(!"sipp not found");
			} else {
				fst_check(status == SWITCH_STATUS_SUCCESS);
...
skiptest:
		...
		FST_TEST_BEGIN(subscribe_auth_check)
		{
			...
			sipp_ret = run_sipp(local_ip_v4, 5064, 6091, "1001", "sipp-scenarios/uac_subscriber.xml", auth_password, "");
			if (sipp_ret < 0 || sipp_ret == 127) {
				fst_check(!"sipp not found");
			} else {
				switch_sleep(100 * 1000);
				...
				sipp_ret = run_sipp(local_ip_v4, 5064, 6090, "1001", "sipp-scenarios/uac_407_subscriber.xml", auth_password, "");
				if (sipp_ret < 0 || sipp_ret == 127) {
					fst_check(!"sipp not found");
				} else {
					switch_sleep(100 * 1000);
