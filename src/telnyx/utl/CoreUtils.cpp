...
//
#include "Telnyx/Persistent/RedisClient.h"
...
static std::string expand_system_dir(const char* dir)
{
  ...
  Telnyx::string_replace(expanded, "${prefix}", TELNYX_PREFIX_DIR);
  Telnyx::string_replace(expanded, "${exec_prefix}", TELNYX_PREFIX_DIR);
  Telnyx::string_replace(expanded, "${datarootdir}", TELNYX_DATA_ROOT_DIR);
  return expanded;
}
...
{
  gSystemPrefix = expand_system_dir(TELNYX_PREFIX_DIR);
  gSystemExecPrefix = expand_system_dir(TELNYX_PREFIX_DIR);
  gSystemBinDir = expand_system_dir(TELNYX_BIN_DIR);
  gSystemSbinDir = expand_system_dir(TELNYX_SBIN_DIR);
  gSystemLibExecDir = expand_system_dir(TELNYX_LIBEXEC_DIR);
  gSystemDataDir = expand_system_dir(TELNYX_DATA_ROOT_DIR);
  gSystemConfDir = expand_system_dir(TELNYX_SYSCONF_DIR);
  gSystemLocalStateDir = expand_system_dir(TELNYX_LOCAL_STATE_DIR);
  gSystemIncludeDir = expand_system_dir(TELNYX_INCLUDE_DIR);
  gSystemLibDir = expand_system_dir(TELNYX_LIB_DIR);
}
